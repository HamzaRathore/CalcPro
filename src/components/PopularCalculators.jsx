...
const PopularCalculators =({ onCalculatorClick }) => {
  ...
  const popularCalculators = CALCULATOR_CATEGORIES.flatMap((cat) =>
    cat.subtypes
      ...
      .map((sub) => ({ ...sub, category: cat.title, icon: cat.icon, gradient: cat.gradient })),
  )
      ...
      <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 mb-16">
        {popularCalculators.slice(0, 8).map((calc, index) => {
          ...
          return (
            <Link to={calc.path}
              key={index}
              className="group bg-white rounded-2xl p-6 shadow-sm hover:shadow-xl transition-all duration-300 cursor-pointer border border-gray-100 hover:border-gray-200"
              // onClick={() => onCalculatorClick(calc.path)}
            >
              <div
                className={`w-12 h-12 bg-gradient-to-r ${calc.gradient} rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}
              >
              ...
              <p className="text-sm text-gray-500 mb-3">{calc.category}</p>
