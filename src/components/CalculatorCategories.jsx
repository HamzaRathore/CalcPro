...
const CalculatorCategories = ({ categories}) => {
  const { searchTerm, isSearching } = useSearch(); 

  // Function to filter subtypes based on search
  const getFilteredSubtypes = (subtypes) => {
    if (!isSearching) return subtypes;
    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-28 pb-20">
      <div className="space-y-12">
        {categories.map((category) => {
          ...
          const filteredSubtypes = getFilteredSubtypes(category.subtypes);
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="hidden md:block md:text-xs md:bg-gray-300 md:font-semibold md:rounded-xl md:px-2 md:py-0.5">
                ...
                {filteredSubtypes.map((subtype, index) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <h4
                          className={`font-medium ${category.textColor} group-hover:text-gray-900 transition-colors`}
                        >
                          {subtype.name}
                        </h4>
                        {subtype.popular && (
